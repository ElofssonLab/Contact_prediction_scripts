...
def write_pdb_atm_record(atom):
    print(
        ...
        .format("ATOM", atom['atm_no'], atom['atm_name'], " ",
                atom['res_name'], atom['chain'], atom['res_no'], " ",
                atom['x'], atom['y'], atom['z'], atom['occ'], atom['B']))
    return
...
def read(pdbfile, chain='', model=1):
    ...
    for line in pdbfile:
        if line.startswith('MODEL') and int(line.strip().split()[-1]) == model:
            ...
            in_model = False
            tail += line
        elif line.startswith('MODEL') and not int(
                line.strip().split()[-1]) == model:
            continue
...
def get_coordinates(pdbfile, chain):
    ...
    for line in pdbfile:
        ...
        if res_dict.keys():
            ...
            min_res_i = res_i
        if res_i > 1000 and len(
                res_dict) < 1000 and min_res_i + len(res_dict) < 1000:
            continue
...
def get_bfactor_area(pdbfile, chain):
    ...
    for line in pdbfile:
        ...
        if b_dict.keys():
            ...
            min_res_i = res_i
        if res_i > 1000 and len(
                b_dict) < 1000 and min_res_i + len(b_dict) < 1000:
            continue
        ...
        if res_i != lastres:
            ...
            fracarea = 0.
        area += atm_record['B']
        fracarea += atm_record['B'] / maxarea[three_to_one[
            atm_record['res_name']]]
...
def get_atom_seq(pdbfile, chain='', model=1, return_lines=False):
    ...
    line_lst = [
        l[1]
        for l in sorted(line_dict.iteritems(), key=operator.itemgetter(0))
    ]
